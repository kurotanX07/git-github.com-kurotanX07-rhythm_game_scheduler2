...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    Future.microtask(() async {
      ...
      try {
        // サンプルデータをFirestoreに追加（初回のみ）
        await eventProvider.seedFirestoreData();
        debugPrint('Attempted to seed Firestore data');
        
        // Firestoreからイベントデータを取得
        await eventProvider.fetchFirestoreEvents();
        debugPrint('Attempted to fetch Firestore data');
      } catch (e) {
        debugPrint('Error during Firestore operations: $e');
        // エラー時はローカルデータを使用（すでにコンストラクタでロード済み）
      }
    });
    
    // ゲームフィルターが変更されたときに、イベントフィルターを更新（既存のコード）
    Future.microtask(() {
          ...
          if (_selectedIndex == 0 && !eventProvider.isSearching)
            IconButton(
              ...
              onPressed: () {
                ...
                eventProvider.fetchFirestoreEvents();
              ...
              Expanded(
                child: Consumer<EventProvider>(
                  builder: (context, eventProvider, child) {
                    return EventList(events: eventProvider.filteredEvents);
                  },
                ),
              ),
            ],
